...
int _setenv(char **args, int overwrite)
{
	...
	envar_length = strlen(args[0]) + strlen(args[1]) + 2;
	create_envar(&env_var, envar_length, args[0], args[1]);
	_env_set_exists(env_var, envar_length, args[0], overwrite);
	if (_env_name_exists(args[0]) != -1)
		_env_set_exists(env_var, envar_length, args[0], overwrite);
	else
